맵을 이동할 때 <MapStateM:> 태그가 있다면 상태 처리에 따른 이벤트 실행을 해줍니다.
기존 알만툴에서 Enter는 자동실행+이벤트 삭제로 구현 가능, Update는 병렬처리로 구현이 가능하지만,
Exit는 완전히 스크립트 영역으로밖에 구현이 불가능하기 때문에 구현하게 되었습니다.
사실 Exit만 만들어도 구현에는 문제 없겠지만 상태 머신을 참고한 형태이므로 안 헷갈리게 통일하는 편이 낫다 판단했습니다.

MapStateManager.js 파일만 다운받아 사용하면 됩니다.
오류 발생 시 Github 또는 newmoonhana@gmail.com으로 제보해주세요. 저도 이 파일 메인으로 사용해야해서 개선해서 사용해야하는지라...
